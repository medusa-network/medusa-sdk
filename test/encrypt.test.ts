...
describe("medusa encryption", () => {
  ...
  it("can decrypt large message", async () => {
    ...
    const msgStr = "None of us is great enough for such a task. But in all circumstances of life, in obscurity or temporary fame, cast in the irons of tyranny or for a time free to express himself, the writer can win the heart of a living community that will justify him, on the one condition that he will accept to the limit of his abilities the two tasks that constitute the greatness of his craft: the service of truth and the service of liberty. Because his task is to unite the greatest possible number of people, his art must not compromise with lies and servitude which, wherever they rule, breed solitude. Whatever our personal weaknesses may be, the nobility of our craft will always be rooted in two commitments, difficult to maintain: the refusal to lie about what one knows and the resistance to oppression.";
    const msgBuff = new TextEncoder().encode(msgStr.padEnd(32, "\0"));
    const c = await suite.onetimeEncrypt(msgBuff, proxy.pubkey, );
    assert.ok(c.isOk());
    const bundle = c._unsafeUnwrap();
    const reencryption = reencrypt(curve, proxy, bob.pubkey, bundle.encrypted_key);
    const m = await suite.onetimeDecrypt(bob.secret, proxy.pubkey, bundle, reencryption);
    assert.ok(m.isOk());
